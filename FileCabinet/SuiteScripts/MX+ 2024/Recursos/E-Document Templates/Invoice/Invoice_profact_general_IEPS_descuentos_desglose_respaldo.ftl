...
<#assign total_envio = transaction.total + envio_total?number + impuesto_envio_total>
<#if customer.custentity_efx_cmp_registra_ieps == true>
...
<#else>
<#assign subtotal_xml = transaction.subtotal?string["0.00"]>
<#if customer.custentity_efx_cmp_registra_ieps == true>
SubTotal="${summary.subtotal?number?string["0.00"]}"
<#else>
<#assign sub_conieps = summary.subtotal?number + total_ieps>
<#assign total_xml = transaction.total>
...
<cfdi:Retenciones>
    ...
    <cfdi:Retencion Base="${desglose.retenciones.base_importe?number?string["0.00"]}" Importe="${(desglose.retenciones.importe?number * -1)?string["0.00"]}" Impuesto="002" TasaOCuota="${desglose.retenciones.rate_div?number?string["0.000000"]}" TipoFactor="Tasa" />
...
<#if customer.custentity_efx_cmp_registra_ieps == true>
    <#if (desglose_cab.ieps_total != "0.00" || desglose_cab.iva_total != "0.00") && desglose_cab.retencion_total != "0.00">
        ...
        <cfdi:Impuestos TotalImpuestosRetenidos="${(desglose_cab.retencion_total?number * -1)?string["0.00"]}" TotalImpuestosTrasladados="${total_impuestos_t?string["0.00"]}">
    <#elseif desglose_cab.retencion_total != "0.00">
        <#assign exentoContador = exentoContador+1>
        <cfdi:Impuestos TotalImpuestosRetenidos="${(desglose_cab.retencion_total?number * -1)?string["0.00"]}">
...
<cfdi:Retenciones>
    ...
    <cfdi:Retencion Importe="${(ret_total?number * -1)?string["0.00"]}" Impuesto="002" />
