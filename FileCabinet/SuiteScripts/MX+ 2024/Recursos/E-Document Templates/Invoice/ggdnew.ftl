...
<#if transaction.custbody_efx_fe_forma_pago?has_content>
    <#assign paymentMethod = transaction.custbody_efx_fe_forma_pago?keep_before(" ")>
<#else>
...
<cfdi:Comprobante
        ...
        <#assign "Add_name" = customer.custentity_efx_fe_addenda>
        ...
        <#assign condicionesPago = "">
        <#if transaction.custbody_numero_orden?has_content>
            <#assign condicionesPago = "PO:"+transaction.custbody_numero_orden>
        </#if>
        ${getAttrPair("CondicionesDePago",condicionesPago)}
        ...
        <#if transaction.shippingcost?length gt 0>
            ...
            <#if transaction.custbody_efx_fe_desglosaieps == true>
            ...
            <#if item.itemtype != "Group" && item.itemtype != "EndGroup">
                <cfdi:Concepto
                        ...
                        <#if ComercioE == false>
                            <#if Add_name=="Amazon">
                                ...
                                <#if item.custcol_efx_fe_amazon_code?has_content>
                                    NoIdentificacion="${item.custcol_efx_fe_amazon_code}"
                                <#else>
                                    NoIdentificacion="${item.item}"
                                </#if>
                    ...
                    <#assign campo_pedimento = item.custcol_detalle_de_pedimento_json>
                    <#if campo_pedimento?has_content>
                        <#assign obj_pedimento = campo_pedimento?eval>
                        <#list obj_pedimento as datos_pedimento>
                            ${getNodePair("cfdi:InformacionAduanera", "NumeroPedimento" ,datos_pedimento.numero)}
                        </#list>
                    </#if>
                ...
                <#else>
                    ...
                    <cfdi:Concepto
                            ...
                            Descuento="${desglose.descuentoSinImpuesto?number?string["0.00"]}">
                        <#else>
                            <#if customItem.totalDiscount?number?abs gt 0>
