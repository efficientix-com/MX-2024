...
${getAttrPair("Folio",transaction.tranid?keep_after("V"))}
...
<cfdi:Emisor ${getAttrPair("Nombre", companyInformation.companyName)} RegimenFiscal="${satCodes.industryType}" Rfc="${companyTaxRegNumber}" />
<cfdi:Receptor
   <#if transaction.custbody_efx_razonsocialafacturar?has_content>
Nombre="${transaction.custbody_efx_razonsocialafacturar}"
 <#else>
Nombre="${customerName}"
 </#if>
                <#if ComercioE == true>
...
NoIdentificacion="${item.custcol_efx_noidentificacion}"
        <#if ComercioE == true>
