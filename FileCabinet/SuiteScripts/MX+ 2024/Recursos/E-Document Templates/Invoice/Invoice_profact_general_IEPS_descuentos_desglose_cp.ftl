            ...
            <#if item.item != articulo_timbrado>
                ...
                <cfdi:Concepto
                        ...
                        <#if desglosa_ieps == true>
                            ...
                            <#if customItem.rate != "">
                                ValorUnitario="${customItem.rate?number?string["0.000000"]}"
                            <#else>
                                <#assign rate_faltante = customItem.amount?number / item.quantity?number>
                                ValorUnitario="${rate_faltante?string["0.000000"]}"
                            </#if>
                        <#else>
                            <#if desglose.ieps.name?has_content>
                                ...
                                <#if valu_linea_sindesglose?has_content>
                                    ValorUnitario="${valu_linea_sindesglose?string["0.000000"]}"
                                <#else>
                    ...
                    <#if customItem.parts?has_content>
                        <#list customItem.parts as part>
                            ...
                            <cfdi:Parte Cantidad="${partItem.quantity?string["0.0"]}" ClaveProdServ="${partSatCodes.itemCode}" Descripcion=<#outputformat "XML">"${partItem.description}"</#outputformat> Importe="${part.amount?number?string["0.00"]}" ValorUnitario="${part.rate?number?string["0.000000"]}"/>
                        </#list>
                    </#if>
                            ...
                            <#if transaction.custbody_efx_fe_desglosa_ga == false>
                                <cfdi:Concepto
                                        ...
                                        <#if desglosa_ieps == true>
                                            ...
                                            <#if customItem.rate != "">
                                                ValorUnitario="${importedesgloses?string["0.000000"]}"
                                            <#else>
                                                <#assign rate_faltante = customItem.amount?number / item.quantity?number>
                                                ValorUnitario="${importedesgloses?string["0.000000"]}"
                                            </#if>
        ...
        <#if transaction.shippingcost?length gt 0>
            ...
            <#if costo_envio_t != "0.00">
                <cfdi:Concepto
                        ...
                        ValorUnitario="${transaction.shippingcost?string["0.000000"]}"
                        Descuento="0.00">
                    <#assign impuesto_envio_rate = transaction.shippingtax1rate>
                                        ...
                                        <#list json_ivas as Iva_rate, Iva_total>
                                            ...
                                            <#if exchangeRate?number gt 0>
                                                ...
                                                <#if iva_tasaocuota==0 && impuestoLineashipp?number gt 0 && numImpuestos==1>
                                                    <cfdi:Traslado Importe="${Iva_total_ex?number?string["0.00"]}" Impuesto="002" <#if impuesto_envio_rate?has_content>TasaOCuota="${(iva_tasaocuota+(impuesto_envio_rate?number/100))?string["0.000000"]}"<#else>TasaOCuota="${iva_tasaocuota?string["0.000000"]}"</#if> TipoFactor="Tasa" />
