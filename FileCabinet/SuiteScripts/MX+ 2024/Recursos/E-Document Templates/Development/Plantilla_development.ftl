...
${getAttrPair("Folio",transaction.traind)}
...
<#assign subtotal_envio = summary.subtotal?number + envio_total?number>
<#assign total_envio = summary.totalAmount?number + envio_total?number + impuesto_envio_total>
...
<#if desglose_cab.retencion_total != "0.00" && summary.includeTransfers == "true">
<cfdi:Impuestos TotalImpuestosRetenidos="${desglose_cab.retencion_total?number?string["0.00"]}" <#if transaction.shippingcost?has_content>TotalImpuestosTrasladados="${(total_tras?number + impuesto_envio_total?number)?string["0.00"]}"><#else>TotalImpuestosTrasladados="${total_tras?number?string["0.00"]}"></#if>
...
<cfdi:Impuestos <#if transaction.shippingcost?has_content>TotalImpuestosTrasladados="${(summary.totalNonWithHoldTaxAmt?number + impuesto_envio_total?number)?string["0.00"]}"<#else>TotalImpuestosTrasladados="${summary.totalNonWithHoldTaxAmt?number?string["0.00"]}" </#if>>
</#if>
...
<cfdi:Traslados>
    ...
    <#if !customTaxItem.taxFactorType?has_content || customTaxItem.taxFactorType != "Exento">
    <#if customTaxItem.taxAmount?number gte 0>
    <cfdi:Traslado <#if transaction.shippingcost?has_content>Importe="${(customTaxItem.taxAmount?number + impuesto_envio_total?number)?string["0.00"]}"<#else>Importe="${customTaxItem.taxAmount?number?string["0.00"]}"</#if> Impuesto="${customTaxItem.satTaxCode}" TasaOCuota="${customTaxItem.taxRate?number?string["0.000000"]}" TipoFactor="${customTaxItem.taxFactorType}" />
</#if>
</#if>
</#list>
</cfdi:Traslados>
