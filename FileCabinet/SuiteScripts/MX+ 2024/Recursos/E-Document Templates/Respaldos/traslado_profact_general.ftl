...
<#if ComercioE == true>
LugarExpedicion="${json_direccion.cfdi.LugarExpedicion}"
<#else>
...
Moneda="${currencyCode}"
<#if ComercioE == true>
${getAttrPair("TipoCambio",json_direccion.cfdi.TipoCambio?string["0.00"])}
<#else>
${getAttrPair("TipoCambio",transaction.origexchangerate)}
</#if>

SubTotal="0.00" TipoDeComprobante="T" Total="0.00" Version="3.3">
...
<cfdi:Emisor ${getAttrPair("Nombre", transaction.location)}
<#if ComercioE == true>
...
<cfdi:Receptor Nombre="${transaction.transferlocation}"
<#if ComercioE == true>
NumRegIdTrib="${CEreceptorNumR}"
ResidenciaFiscal="${CEreceptorResidF}"
</#if>
<#if ComercioE == true>
Rfc="${json_direccion.receptor.Rfc}" UsoCFDI="${json_direccion.receptor.UsoCFDI}" />
<#else>
...
<cfdi:Conceptos>
    ...
    <#assign "itemSatCodes" = json_direccion.articulos[cuentaLineas]>
    <#if customItem.type == "Group" || customItem.type == "Kit">
    <#assign "itemSatUnitCode" = "H87">
    <#else>
    <#assign "itemSatUnitCode" = json_direccion.articulos[cuentaLineas]>
</#if>
<cfdi:Concepto
...
${getAttrPair("ClaveProdServ",(itemSatCodes.claveprodserv)!"")!""}
${getAttrPair("ClaveUnidad",itemSatUnitCode.claveunidad)!""}
Descripcion="${item.description}"
<#if ComercioE == true>
<#assign importeLinea = item.amount>
<#assign cantidadLinea = item.quantity>
<#assign VunitarioLinea = item.rate>
Importe="${importeLinea?string["0.00"]}"
ValorUnitario="${VunitarioLinea?string["0.00"]}">
<#else>
Importe="${customItem.amount?number?string["0.00"]}"
ValorUnitario="0.00">
</#if>
${getNodePair("cfdi:InformacionAduanera", "NumeroPedimento" ,customItem.custcol_mx_txn_line_sat_cust_req_num)}
${getNodePair("cfdi:CuentaPredial", "Numero" ,customItem.custcol_mx_txn_line_sat_cadastre_id)}
...
<cce11:Mercancias>
    <#list transaction.item as customItem>
    <#assign "item" = transaction.item[customItem.line?number]>
    <cce11:Mercancia NoIdentificacion="${customItem.custcol_efx_fe_upc_code}" FraccionArancelaria="${customItem.custcol_efx_fe_ce_farancel_code}" CantidadAduana="${customItem.custcol_efx_fe_ce_cant_aduana?replace(',','')?number?string["0.000"]}" UnidadAduana="${customItem.custcol_efx_fe_unit_code_ce}" ValorUnitarioAduana="${customItem.custcol_efx_fe_ce_val_uni_aduana?replace(',','')?number?string["0.00"]}" ValorDolares="${customItem.custcol_efx_fe_ce_val_dolares?replace(',','')?number?string["0.00"]}">
    <#if customItem.custcol_efx_fe_ce_des_especificas?has_content>
    <cce11:DescripcionesEspecificas Marca="${customItem.custcol_efx_fe_ce_des_especificas}" NumeroSerie="${customItem.custcol_efx_fe_ce_des_numero_serie}" Modelo="${customItem.custcol_efx_fe_ce_des_modelo}"/>
</#if>
</cce11:Mercancia>
