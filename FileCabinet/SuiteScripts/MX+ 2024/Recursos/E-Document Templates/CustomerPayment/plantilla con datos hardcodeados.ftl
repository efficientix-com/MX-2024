...
<cfdi:Comprobante
        ...
        <cfdi:Receptor Nombre="${transaction.custbody_efx_fe_kiosko_name}"
                <#if ComercioE == true>
                    ...
                    ResidenciaFiscal="${CEreceptorResidF}"
                       ...
                       DomicilioFiscalReceptor="${transaction.custbody_efx_fe_kiosko_rfc}"
    ...
    <cfdi:Conceptos>
        <#list custom.items as customItem>
            <#assign "item" = transaction.item[customItem.line?number]>
            <#if item.itemtype != "Discount" && item.quantity gt 0>
                <cfdi:Concepto
                    Cantidad="${item.quantity?string["0.000000"]}"
                    ClaveProdServ="${item.custcol_efx_fe_clave_producto_sat}"
                    ClaveUnidad="E48"
                ...
                <cfdi:Impuestos>
                        <cfdi:Traslados>
                            <cfdi:Traslado Base="${item.amount?string["0.00"]}" Importe="0.00" Impuesto="002" TasaOCuota="0.000000" TipoFactor="Tasa" />
                        </cfdi:Traslados>
                    </cfdi:Impuestos>
                </cfdi:Concepto>
            </#if>
    ...
    <cfdi:Impuestos TotalImpuestosTrasladados="0.00">
        <cfdi:Traslados>
            <cfdi:Traslado Base="48.00" Importe="0.00" Impuesto="002" TasaOCuota="0.000000" TipoFactor="Tasa" />
        </cfdi:Traslados>
    </cfdi:Impuestos>
                                    ...
                                    <cfdi:Traslados>
                                        ...
                                        <#list json_ivas as Iva_rate, Iva_total>
                                            ...
                                            <#if exchangeRate?number gt 0>
                                                ...
                                                <#if iva_tasaocuota==0 && impuestoLineashipp?number gt 0>
                                                    <cfdi:Traslado Base="${json_ivas_base[Iva_rate]}" Importe="${Iva_total_ex?number?string["0.00"]}" Impuesto="002" <#if impuesto_envio_rate?has_content>TasaOCuota="${(iva_tasaocuota+(impuesto_envio_rate?number/100))?string["0.000000"]}"<#else>TasaOCuota="${iva_tasaocuota?string["0.000000"]}"</#if> TipoFactor="Tasa" />
                                                <#else>
                                                    <cfdi:Traslado Base="${json_ivas_base[Iva_rate]}" Importe="${Iva_total_ex?number?string["0.00"]}" Impuesto="002" TasaOCuota="${iva_tasaocuota?string["0.000000"]}" TipoFactor="Tasa" />
                                                </#if>

                                            <#else>

                                                <cfdi:Traslado Base="${json_ivas_base[Iva_rate]}" Importe="${(Iva_total?number+impuestoLineashipp?number)?string["0.00"]}" Impuesto="002" TasaOCuota="${iva_tasaocuota?string["0.000000"]}" TipoFactor="Tasa" />
