 ...
 */
define(['N/record', 'N/render', 'N/search', 'N/runtime', '../Utilities/libcustomitems', '../Utilities/summary', 'N/file', 'N/xml', 'N/encode', 'N/https', 'N/http', '../Utilities/pagodata', 'N/config', '../Utilities/XmlToPdf', 'N/format', '../Utilities/EFX_FE_Lib', 'N/url'/*,'../EFX_FE_Config_Parameters'*/],
    /**
     * @param{https} https
     * @param{record} record
     * @param{search} search
     */
    function (record, render, search, runtime, customItems, summaryCalc, file, xml, encode, https, http, pagodata, config, XmlToPdf, format, libCFDI, url/*,runtimeObj*/) {
        ...
        function onRequest(context) {
            ...
            var nombre_kiosko = context.request.parameters.razon_social || '';
            var rfc_kiosko = context.request.parameters.rfc || '';
            var codigo_postal_kiosko = context.request.parameters.codigo_postal || '';
            var regimenFiscal_kiosko = context.request.parameters.regimenFiscal || '';
            var trantype = context.request.parameters.trantype || '';

            log.audit({ title: 'id_transaccion', details: id_transaccion });
            if (id_transaccion) {
                // Carga factura con id que viene en parámetros
                var recordobj = record.load({
                    type: trantype,
                    id: id_transaccion
                });
                // Traer plantilla de facturacion
                ...
                log.audit({ title: 'tran_uuid', details: tran_uuid });
                if (!tran_uuid) {
                    log.audit({ title: 'folderBaseSubsidiaria', details: folderBaseSubsidiaria });
                    // if (folderBaseSubsidiaria) {
                    ...
                    if (!tran_sendingmethod || !id_template) {
                        ...
                        var clientId = '';
                        var record_type = trantype;

                        //Es factura por lo que siempre será "entity"
                    ...
                    // Obtiene credenciales del PAC usuario y url que coincida con normalmente "profact document package"
                    var credentials_pac = getCredentialsPac(e_package_name, SUBSIDIARIES, recordobj);
                    log.audit({ title: 'credentials_pac', details: credentials_pac });
                    // Carga plantilla y obten su contenido
                    var templateobj = record.load({
                        type: 'customrecord_psg_ei_template',
                        id: id_template,
                    });
                    var template = templateobj.getValue({ fieldId: 'custrecord_psg_ei_template_content' });
                    // Custom object para renderizar plantilla (se trae los datos a poner en plantilla)
                    var recordObjrecord = recordobj;
                    log.audit({ title: 'nombre_kiosko', details: nombre_kiosko });
                    var result = obtenercustomobject(recordObjrecord, {}, trantype, '', '', id_transaccion, '', '', '', nombre_kiosko, rfc_kiosko, codigo_postal_kiosko, regimenFiscal_kiosko);
                    log.audit({ title: 'result', details: result });
                    log.audit({ title: 'result.nombre_kiosko', details: result.nombre_kiosko });
                    var customJson = {
                        customDataSources: [
                            {
                    ...
                    // Coloca valores CustomJson en plantilla y timbra documento
                    var respuesta_return = renderWithValues(customJson, template, recordObjrecord, entityObj, templateobj, id_transaccion, credentials_pac.user_pac, credentials_pac.url_pac, credentials_pac.template_invoice_pac, credentials_pac.tax_id_pac, result, id_cliente_tran, idPropietario, nombre_kiosko, rfc_kiosko, codigo_postal_kiosko, regimenFiscal_kiosko, folderBase,trantype);
                    context.response.write({
                        output: JSON.stringify(respuesta_return)
                    });
                    // }
                }
            }
        ...
        function renderWithValues(customJson, template, recordObjrecord, entityObj, templateobj, id_transaccion, user_pac, url_pac, template_invoice_pac, tax_id_pac, result, id_cliente_tran, idPropietario, nombre_kiosko, rfc_kiosko, codigo_postal_kiosko, regimenFiscal_kiosko, folderBase,trantype) {
            try {
                ...
                if (content) {
                    try {
                        ...
                        log.error({ title: 'Error occurred in submitting field of content of xml', details: err });
                    }
                    if (validacion.validado) {
                        var xmlResultData = timbraDocumento(content, user_pac, url_pac);
                        ...
                        if (xmlResultData.success == true) {
                            ...
                            respuesta.success = false;
                            respuesta.error_details = catalogaError(xmlResultData.error);

                            // log de errores en e-document
                            var mensaje_generacion = 'Mensaje: ' + xmlResultData.error;
                            var log_record = record.create({
                                type: 'customrecord_psg_ei_audit_trail',
                                isDynamic: true
                            });
                        ...
                        var objRespuesta = obtenCFDIDatos(anyType);
                        var xmlTimbrado = '';
                        ...
                        if (!objRespuesta.certData.existError) {
                            ...
                            //     },
                            // });
                            nombreXml = 'Factura' + '_' + id_transaccion + '.xml';
                            ...
                            var pdf_tran_id = '';
                            try {
                                pdf_tran_id = generarPDFTimbrado(recordObjrecord, entityObj, objRespuesta, template_invoice_pac, tax_id_pac, id_transaccion, result, folderBase, resultDirecciones);

                            } catch (err) {
                                log.error({ title: 'Error occurred in trying to generarPDFTimbrado', details: err });
                            }
                            ...
                            if (fileXmlIdTimbrado) {
                                ...
                                //             custbody_fb_tp_xml_data: JSON.stringify(obj_data),
                                //             custbody_efx_fe_kiosko_name: nombre_kiosko,
                                //             custbody_efx_fe_kiosko_rsocial: nombre_kiosko,
                                //             custbody_efx_fe_kiosko_zip: codigo_postal_kiosko,
                                //             custbody_efx_fe_kiosko_rfc: rfc_kiosko
                                //         },
                                //         options: {
                                //             enableSourcing: false,
                                //             ignoreMandatoryFields: true
                                ...
                                // } catch (err) {
                                //     log.error({ title: 'Error occurred in trying to submit record fileXMLIdTimbrado', details: err });
                                // }
                                // Manda respuesta
                                ...
                                respuesta.tranid = id_transaccion;
                                respuesta.trantype = trantype;
                                respuesta.error_details = '';
                                // inicia prueba de envio de datos que van en factura para guardarlo en connection porque da error de tiempo excedido
                                respuesta.custbody_psg_ei_certified_edoc = fileXmlIdTimbrado;
                                respuesta.custbody_psg_ei_status = 3;
                                respuesta.custbody_mx_cfdi_uuid = objRespuesta.certData.custbody_mx_cfdi_uuid;
                                respuesta.custbody_mx_cfdi_certify_timestamp = objRespuesta.certData.custbody_mx_cfdi_certify_timestamp;
                                respuesta.custbody_mx_cfdi_signature = objRespuesta.certData.custbody_mx_cfdi_signature;
                                respuesta.custbody_mx_cfdi_sat_signature = objRespuesta.certData.custbody_mx_cfdi_sat_signature;
                                respuesta.custbody_mx_cfdi_sat_serial = objRespuesta.certData.custbody_mx_cfdi_sat_serial;
                                respuesta.custbody_mx_cfdi_cadena_original = objRespuesta.certData.custbody_mx_cfdi_cadena_original;
                                respuesta.custbody_mx_cfdi_issuer_serial = objRespuesta.certData.custbody_mx_cfdi_issuer_serial;
                                respuesta.custbody_mx_cfdi_qr_code = objRespuesta.certData.custbody_mx_cfdi_qr_code;
                                respuesta.custbody_edoc_generated_pdf = pdf_tran_id;
                                respuesta.custbody_fb_tp_xml_data = JSON.stringify(obj_data);
                                respuesta.custbody_efx_fe_kiosko_name = nombre_kiosko;
                                respuesta.custbody_efx_fe_kiosko_rsocial = nombre_kiosko;
                                respuesta.custbody_efx_fe_kiosko_zip = codigo_postal_kiosko;
                                respuesta.custbody_efx_fe_kiosko_regfiscal = regimenFiscal_kiosko;
                                respuesta.custbody_efx_fe_kiosko_rfc = rfc_kiosko
                            }

                        } else {
                            log.error({ title: 'Error en certificado', details: 'hubo error de certificado' });
                            respuesta.success = false;
                            respuesta.xml_generated = '';
                            respuesta.xml_certified = '';
                            respuesta.pdf_generated = '';
                            ...
                            // log de errores en e-document
                            var mensaje_generacion = 'Mensaje: hubo errores de certificado';
                        ...
                        try {
                            record.submitFields({
                                ...
                                values: {
                                    ...
                                    custbody_efx_fe_kiosko_rsocial: nombre_kiosko,
                                    custbody_efx_fe_kiosko_zip: codigo_postal_kiosko,
                                    custbody_efx_fe_kiosko_regfiscal: regimenFiscal_kiosko,
                                    custbody_efx_fe_kiosko_rfc: rfc_kiosko
                                },
                                options: {
                                    enableSourcing: false,
                            ...
                            log.error({ title: 'Error occurred in submitting custbody_psg_ei_status', details: err });
                        }
                    }
                }
                return respuesta;
                ...
                log.error({ title: 'Error occurred in renderWithValues', details: err });
                var mensaje_generacion = 'Mensaje: ' + err;
                var log_record = record.create({
                    type: 'customrecord_psg_ei_audit_trail',
                    isDynamic: true
                });
        ...
        function catalogaError(xmlErrorMsg) {
            var msg_to_return = xmlErrorMsg;
            try {
                var error_matrix = [
                    { id: 'CFDI40143', mensaje: 'El nombre (Denominación/Razón Social) no coincide con el RFC ingresado' },
                    { id: 'CFDI40145', mensaje: 'El nombre (Denominación/Razón Social) no coincide con el RFC ingresado' },
                    { id: 'CFDI40147', mensaje: 'El Código Postal ingresado debe encontrarse en la lista de RFC inscritos no cancelados en el SAT' },
                    { id: 'CFDI40148', mensaje: 'El Código Postal ingresado no coincide con el RFC' }
                ];
                if (xmlErrorMsg) {
                    error_matrix.forEach(element => {
                        if (xmlErrorMsg.includes(element.id)) {
                            msg_to_return = element.mensaje;
                        }
                    });
                }
                return msg_to_return
            } catch (err) {
                log.error({ title: 'Error occurred in catalogaError', details: err });
                return msg_to_return

            }
        }
        function getCredentialsPac(e_package_name, SUBSIDIARIES, recordobj) {
            var data_to_return = {
                success: false,
                msg: '',
                tax_id_pac: '',
            ...
            try {
                ...
                data_to_return.tax_id_pac = resultado[0].getValue({ name: 'custrecord_mx_pacinfo_taxid' });
                data_to_return.user_pac = resultado[0].getValue({ name: 'custrecord_mx_pacinfo_username' });
                data_to_return.url_pac = resultado[0].getValue({ name: 'custrecord_mx_pacinfo_url' });
                data_to_return.template_invoice_pac = resultado[0].getValue({ name: 'custrecord_mx_invoice_pdf_tmpl' });
                return data_to_return;

            } catch (err) {
                log.error({ title: 'Error occurred in getCredentialsPac', details: err });
                data_to_return.success = false;
                data_to_return.msg = 'Error al buscar configuración de PAC en instancia'
                return data_to_return;
            }

        }
        function obtenercustomobject(recordObjrecord, recordsLoaded, tipo_transaccion, tipo_transaccion_gbl, tipo_cp, id_transaccion, pagoCompensacionBody, pagoCompensacion, tipo_transaccion_gbl_tipo, nombre_kiosko, rfc_kiosko, codigo_postal_kiosko, regimenFiscal_kiosko) {
            ...
            var obj_main = {
                ...
                nombre_kiosko: nombre_kiosko,
                rfc_kiosko: rfc_kiosko,
                codigo_postal_kiosko: codigo_postal_kiosko,
                regimenFiscal_kiosko: regimenFiscal_kiosko,
        ...
        function generarPDFTimbrado(recordObjrecord, entityObj, objRespuesta, template_invoice_pac, tax_id_pac, tran_tranid, result, idFolder, resultDirecciones) {
        ...
        function timbraDocumento(xmlDocument, user_pac, url_pac) {
            ...
            try {
                ...
                // se utiliza SmarterWeb
                var tokenResult = getTokenSW(user_pac, '', url_pac);
        ...
        function obtenCFDIDatos(anyType) {
            var objRespuesta = {};
            // Es smarterWeb
