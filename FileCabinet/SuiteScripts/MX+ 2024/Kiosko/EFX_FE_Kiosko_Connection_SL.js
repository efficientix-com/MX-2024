...
define(['N/https', 'N/record', 'N/search', 'N/url', 'N/file', 'N/format', 'N/runtime', 'N/config'],
    ...
    function (https, record, search, urlMod, file, format, runtime, config) {
            ...
            if (rfc_cliente != 'XAXX010101000') {
                if (getinfo_ticket == 'T') {
                    if (accion_ticket == 'T') {
                        ...
                        //
                        // var dateparse = format.format({
                        //     value: fecha_ticket,
                        //     type: format.Type.DATE
                        // });
                        //
                        // log.audit({title:'dateparse',details:dateparse});
                        ...
                        var search_ticket = search.create({
                            ...
                            // [["mainline",search.Operator.IS,"T"],"AND",["amount",search.Operator.EQUALTO,monto_ticket],"AND",["trandate",search.Operator.ON,fecha_ticket],"AND",['numbertext', search.Operator.IS, tranid_ticket]]
                            ...
                            //     //datos cliente
                            //     search.createColumn({name: "custentity_mx_rfc", join: "customer"}),
                            //     search.createColumn({name: "isperson", join: "customer"}),
                            //     search.createColumn({name: "custentity_efx_cmp_registra_ieps", join: "customer"}),
                        ...
                        if (resultado.length > 0) {
                            ...
                            if (!timbrado && !id_xml) {
                                ...
                                if (resultado.length > 0 && rfc_cliente != 'XAXX010101000') {
                                    ...
                                    log.audit({ title: 'info_cliente_obj 🐙', details: info_cliente_obj });
                                    regimenFiscal_actual = info_cliente_obj.cliente.regFiscal;

                                    log.audit({ title: 'regimenFiscal_actual 🐙🐙🐙🐙', details: regimenFiscal_actual });
                                    // Guardar en campo auxiliar los datos que no se quieren actualizar (código postal y regimen fiscal)
                                    if (info_cliente_obj.cliente.id) {
                                        log.debug({ title: 'ID CLIENTE ubicado', details: 'Entra a actualizar record de cliente a almacenar datos anteriores' });
                                        var obj_old_client_values = {
                                            regimenFiscal_text: regimenFiscal_actual.text,
                                            regimenFiscal_value: regimenFiscal_actual.value,
                                            nombre: info_cliente_obj.cliente.nombre,
                                            apellido: info_cliente_obj.cliente.apellido,
                                            compania: info_cliente_obj.cliente.compania,
                                            codigo_postal: info_cliente_obj.direccion.cp,
                                            calle: info_cliente_obj.direccion.calle,
                                            n_exterior: info_cliente_obj.direccion.n_exterior,
                                            n_interior: info_cliente_obj.direccion.n_interior,
                                            pais: info_cliente_obj.direccion.pais,
                                            estado: info_cliente_obj.direccion.estado,
                                            municipio: info_cliente_obj.direccion.municipio,
                                            colonia: info_cliente_obj.direccion.colonia,

                                        };
                                        var cliente_rcrd = record.load({
                                            type: "customer",
                                            id: info_cliente_obj.cliente.id

                                        });
                                        cliente_rcrd.setValue({
                                            fieldId: 'custentity_mx_plus_kiosko_old_values',
                                            value: JSON.stringify(obj_old_client_values),
                ...
                if (context.request.body) {
                    ...
                    if (!body.transaccion.isClient || body.cliente.rfc == 'XAXX010101000' || (global_fact && body.cliente.rfc != 'XAXX010101000')) {
                        ...
                        if ((!relacionada_fact && !cliente_k_fact) || (!relacionada_fact && cliente_k_fact && global_fact)) {
                            if ((uuid_fact || certificado_fact) && (!cliente_k_fact || (!relacionada_fact && cliente_k_fact && global_fact))) {
                                log.audit({ title: 'NoTieneRelacionada', details: '' });
                                log.audit({ title: 'cliente_objeto 💕', details: cliente_objeto });
                            ...
                            if (cliente_k_fact) {
                                log.debug({ title: 'Entra a cliente_k_fact lin 951', details: '' });
                                var id_cliente = '';
                                if (!cliente_objeto) {
                                    id_cliente = buscarCliente(body, body.cliente.rfc, 'T', SUBSIDIARIES);
                                } else {
                                ...
                                // investiga regimen fiscal y id_cliente
                                regimenFiscal_kiosko = nueva_tran_obj.getValue({ fieldId: 'custbody_efx_fe_kiosko_regfiscal' });
                                id_cliente = nueva_tran_obj.getValue({ fieldId: 'custbody_efx_fe_kiosko_customer' });
                                log.audit({ title: 'regimenFiscal_kiosko🍣', details: regimenFiscal_kiosko });
                                log.audit({ title: 'id_cliente🍣🍣', details: id_cliente });
                                var id_cliente2 = '';
                                if (!cliente_objeto) {
                                    id_cliente2 = buscarCliente(body, body.cliente.rfc, 'T', SUBSIDIARIES);
                                } else {
                                    id_cliente2 = cliente_objeto;
                                }
                                log.audit({ title: 'id_cliente2 3 doritos después🍣🍣🍣', details: id_cliente2 });
                                ...
                                if (rfcclientekiosko) {
                                    if (body.cliente.rfc != rfcclientekiosko) {
                                        ...
                                        // ingresa datos de kiosko
                                        // log.debug({ title: 'Entra a actualizar transaccion existente de kiosko🍡', details: '🍡🍡' });
                                        // var razon_social_to_send = '';
                                        // if (body.cliente.nombre !== '') {
                                        //     var name_txt = body.cliente.nombre + ' ' + body.cliente.apellido
                                        //     // removerAcentos también quita dobles espacios
                                        //     razon_social_to_send = removerAcentos(name_txt.toUpperCase());
                                        // } else {
                                        //     // It's a company (persona moral)
                                        //     var company_txt = body.cliente.compania
                                        //     razon_social_to_send = removerAcentos(company_txt.toUpperCase());
                                        // }
                                        // nueva_tran_obj.setValue({
                                        //     fieldId: 'custbody_efx_fe_kiosko_name',
                                        //     value: razon_social_to_send,
                                        //     ignoreFieldChange: true
                                        // });
                                        // nueva_tran_obj.setValue({
                                        ...
                                        //     value: body.direccion.cp,
                                        //     ignoreFieldChange: true
                                        // });
                                        // nueva_tran_obj.setValue({
                                        //     fieldId: 'custbody_efx_fe_kiosko_rfc',
                                        //     value: body.cliente.rfc,
                                        //     ignoreFieldChange: true
                                        // });
                                        // nueva_tran_obj.setValue({
                                        //     fieldId: 'custbody_efx_fe_kiosko_customer',
                                        ...
                                        //     value: body.cliente.regFiscal.value,
                                        //     ignoreFieldChange: true
                                        // });
                                        // nueva_tran_obj.setValue({
                                        //     fieldId: 'custbody_efx_fe_kiosko_regfiscal',
                                        //     value: id_cliente_aux,
                                        //     ignoreFieldChange: true
                                        // });
                        ...
                        if (id_factura_nueva) {
                            try {
                                if (!generado_pdf && !generado_xml) {
                                    ...
                                    log.audit({ title: 'id_cliente', details: id_cliente });
                                    log.audit({ title: 'id_cliente2', details: id_cliente2 });
                                    log.audit({ title: 'cliente_obj', details: cliente_obj });
                                    // Considerado caso de factura previamente creada pero el cliente es nuevo
                                    // if (!id_cliente2) {

                                    //     var crearXml = crearXML(id_factura_nueva, record_tipo.type, '', body, cliente_obj);
                                    // } else {
                                    //     var crearXml = crearXML(id_factura_nueva, record_tipo.type, '', body, id_cliente2);
                                    ...
                                    if (crearXml.code == 200) {
                                        var body_crearXML = JSON.parse(crearXml.body);
                                        log.audit({ title: 'body_crearXML 🐛', details: body_crearXML });
                                        log.audit({ title: 'body_crearXML typeof 🐛', details: typeof body_crearXML });
                                        try {
                                            // Solo si el timbrado fue exitoso, manda correo
                                            log.audit({ title: 'body_crearXML.success 🐛🐛', details: body_crearXML.success });
                                            if (body_crearXML.success == true) {
                                                log.audit({ title: 'body_crearXML.success 🐛🐛🐛', details: body_crearXML.success });
                                                // guarda registro de factura con informacion del timbre
                                                try {
                                                    var record_stamped = record.load({
                                                        type: body_crearXML.trantype,
                                                        id: body_crearXML.tranid
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_psg_ei_certified_edoc',
                                                        value: body_crearXML.custbody_psg_ei_certified_edoc,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_psg_ei_status',
                                                        value: body_crearXML.custbody_psg_ei_status,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_mx_cfdi_uuid',
                                                        value: body_crearXML.custbody_mx_cfdi_uuid,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_mx_cfdi_certify_timestamp',
                                                        value: body_crearXML.custbody_mx_cfdi_certify_timestamp,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_mx_cfdi_signature',
                                                        value: body_crearXML.custbody_mx_cfdi_signature,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_mx_cfdi_sat_serial',
                                                        value: body_crearXML.custbody_mx_cfdi_sat_serial,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_mx_cfdi_cadena_original',
                                                        value: body_crearXML.custbody_mx_cfdi_cadena_original,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_mx_cfdi_issuer_serial',
                                                        value: body_crearXML.custbody_mx_cfdi_issuer_serial,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_mx_cfdi_qr_code',
                                                        value: body_crearXML.custbody_mx_cfdi_qr_code,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_edoc_generated_pdf',
                                                        value: body_crearXML.custbody_edoc_generated_pdf,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.setValue({
                                                        fieldId: 'custbody_fb_tp_xml_data',
                                                        value: body_crearXML.custbody_fb_tp_xml_data,
                                                        ignoreFieldChange: true
                                                    });
                                                    record_stamped.save({
                                                        enableSourcing: false,
                                                    ...
                                                    log.debug({title:'termina de guardar',details:'linea 1356 ❤️'});

                                                } catch (err) {
                                                    log.error({ title: 'Error occurred in trying to submit fields of invoice stamped', details: err });
                                                }

                                                sendToMail(id_factura_nueva,body_crearXML.trantype)
                                            }
                                            // Carga registro de cliente para leer campo auxiliar de valores anteriores y cambiar código postal al anterior y regimen fiscal (usar value)
                                            try {
                                                if (body.cliente.existe) {
                                                    var cliente_recordRegresaValoresAnteriores = record.load({
                                                        type: "customer",
                                                        id: body.cliente.id,

                                                    });
                                                    var oldvalues_field = cliente_recordRegresaValoresAnteriores.getValue({
                                                        fieldId: "custentity_mx_plus_kiosko_old_values"
                                                    });
                                                    var obj_oldvalues = JSON.parse(oldvalues_field);
                                                    log.audit({ title: 'obj_oldvalues 🐬🐬', details: obj_oldvalues });
                                                    cliente_recordRegresaValoresAnteriores.setValue({
                                                        fieldId: "custentity_mx_sat_industry_type",
                                                        value: obj_oldvalues.regimenFiscal_value,
                                                        ignoreFieldChange: true
                                                    });

                                                    cliente_recordRegresaValoresAnteriores.setValue({
                                                        fieldId: "custentity_mx_sat_registered_name",
                                                        value: obj_oldvalues.compania,
                                                        ignoreFieldChange: true
                                                    });

                                                    // Pasar a los valores anteriores lo de direccion en addressbook de cliente
                                                    var numLines = cliente_recordRegresaValoresAnteriores.getLineCount({
                                                        sublistId: 'addressbook'
                                                    });
                                                    for (var n = 0; n < numLines; n++) {
                                                        var direccion_subrecord = cliente_recordRegresaValoresAnteriores.getSublistSubrecord({
                                                            sublistId: 'addressbook',
                                                            fieldId: 'addressbookaddress',
                                                            line: n
                                                        });
                                                        var isBillingAddress = cliente_recordRegresaValoresAnteriores.getSublistValue({
                                                            sublistId: 'addressbook',
                                                            fieldId: 'defaultbilling',
                                                            line: n
                                                        });
                                                        if (isBillingAddress) {
                                                            direccion_subrecord.setValue({
                                                                fieldId: 'addr2',
                                                                value: obj_oldvalues.calle + obj_oldvalues.n_exterior
                                                            });

                                                            direccion_subrecord.setValue({
                                                                fieldId: 'addr1',
                                                                value: obj_oldvalues.nombre
                                                            });

                                                            direccion_subrecord.setValue({
                                                                fieldId: 'custrecord_streetname',
                                                                value: obj_oldvalues.calle
                                                            });

                                                            direccion_subrecord.setValue({
                                                                fieldId: 'custrecord_streetnum',
                                                                value: obj_oldvalues.n_exterior
                                                            });

                                                            direccion_subrecord.setValue({
                                                                fieldId: 'custrecord_colonia',
                                                                value: obj_oldvalues.colonia
                                                            });


                                                            direccion_subrecord.setValue({
                                                                fieldId: 'country',
                                                                value: obj_oldvalues.pais
                                                            });

                                                            direccion_subrecord.setValue({
                                                                fieldId: 'state',
                                                                value: obj_oldvalues.estado
                                                            });

                                                            direccion_subrecord.setValue({
                                                                fieldId: 'city',
                                                                value: obj_oldvalues.municipio
                                                            });

                                                            direccion_subrecord.setValue({
                                                                fieldId: 'zip',
                                                                value: obj_oldvalues.codigo_postal
                                                            });
                                                        }
                                                    }
                                                    cliente_recordRegresaValoresAnteriores.save({
                                                        enableSourcing: false,
                                                        ignoreMandatoryFields: true
                                                    });
                                                    log.audit({ title: 'Finished setting the obj_oldvalues 🐬🐬🐬', details: true });
                                                }

                                            } catch (err) {
                                                log.error({ title: 'Error occurred on setting the old values for client', details: err });
                                            }
                                        ...
                                        if (id_pdf && id_xml) {
                                            ...
                                            if (crearXml.body) {
                                                ...
                                                if (parsebody.error_details) {
                                                    ...
                                                    // MOD: error details tiene el texto como tal
                            ...
                            if (cliente_k_fact) {
                                try {
                                    if (!generado_pdf && !generado_xml) {
                                        log.audit({ title: 'No esta Timbrada', details: '' });
                                        var crearXml = crearXML(record_tipo.id, record_tipo.type, cliente_k_fact, body);
                                        log.audit({ title: 'crearXml_de_factura_existente', details: crearXml });

                                        if (crearXml.code == 200) {
                                            var body_crearXML = JSON.parse(crearXml.body);
                                            log.audit({ title: 'body_crearXML 🐛3', details: body_crearXML });
                                            log.audit({ title: 'body_crearXML typeof 🐛3', details: typeof body_crearXML });
                                            try {
                                                log.audit({ title: 'body_crearXML.success 🐛🐛3', details: body_crearXML.success });
                                            if (body_crearXML.success == true) {
                                                log.audit({ title: 'body_crearXML.success 🐛🐛🐛3', details: body_crearXML.success });
                                                // guarda registro de factura con informacion del timbre
                                                try {
                                                    ...
                                                    log.debug({title:'termina de guardar',details:'linea 1681 🐧'});
                                            ...
                                            var id_pdf = body_crearXML.pdf_generated;
                                            var id_xml = body_crearXML.xml_certified;
                                            // var id_pdf = nueva_tran_obj.getValue({ fieldId: 'custbody_edoc_generated_pdf' });
                                            // var id_xml = nueva_tran_obj.getValue({ fieldId: 'custbody_psg_ei_certified_edoc' });
                        ...
                        try {
                            if (!generado_pdf || !generado_xml) {
                                var crearXml = crearXML(body.transaccion.transId, record_tipo.type, 'T', body);
                                ...
                                if (crearXml.code == 200) {
                                    ...
                                    if (body_crearXML.success == true) {
                                        ...
                                        try {
                                            ...
                                            log.debug({title:'termina de guardar transacción',details:'linea 2029 🕸️🕸️'});

                                        } catch (err) {
                                            log.error({ title: 'Error occurred in trying to submit fields of invoice stamped 2', details: err });
                                        }
                                    }
        ...
        function updateInvoiceKioskoInformation(transaction_id, body, razon_social, clienteId,trantype) {
            try {
                ...
                invoice_rcrd.setValue({
                    ...
                    value: body.direccion.calle + ' ' + body.direccion.n_exterior + ' ' + body.direccion.n_interior + '\n' + body.direccion.colonia + '\n' + body.direccion.cp + ' ' + body.direccion.municipio + ', ' + body.direccion.estado + '\n' + body.direccion.pais,
                    ignoreFieldChange: true
                });
                invoice_rcrd.setValue({
                    fieldId: 'custbody_efx_fe_kiosko_rsocial',
                ...
                log.error({ title: 'Error occurred in updateInvoiceKioskoInformation', details: err });
            }
        }

        function crearXML(tranid, trantype, cliente_k_fact, body_peticion, clienteId) {
            log.audit({ title: 'tranid', details: tranid });
            log.audit({ title: 'trantype', details: trantype });
            log.audit({ title: 'body_peticion 🔔', details: body_peticion });
            log.audit({ title: 'clienteId 🔔', details: clienteId });
            var razon_social_to_send = '';
            if (body_peticion.cliente.nombre !== '') {
                if (body_peticion.cliente.razonSocial !== '') {
                    var name_txt = body_peticion.cliente.razonSocial
                } else {
                    var name_txt = body_peticion.cliente.nombre + ' ' + body_peticion.cliente.apellido
                }
                // removerAcentos también quita dobles espacios
                razon_social_to_send = removerAcentos(name_txt.toUpperCase());
            } else {
                // It's a company (persona moral)
                if (body_peticion.cliente.razonSocial !== '') {
                    var company_txt = body_peticion.cliente.razonSocial
                } else {
                    var company_txt = body_peticion.cliente.compania
                }
                razon_social_to_send = removerAcentos(company_txt.toUpperCase());
            }
            log.audit({ title: 'razon_social_to_send 🐝', details: razon_social_to_send });
            ...
            var regFiscal = body_peticion.cliente.regFiscal.text
            if (!body_peticion.cliente.regFiscal.text) {
                var client = record.load({
                    type: tipodeTran,
                    id: tranid,

                });
                regFiscal = client.getText({
                    fieldId: "custbody_efx_fe_kiosko_regfiscal"
                });
                // Si es creada por primera vez la factura y cliente, no es necesario hacer actualizacion en factura
                if (clienteId) {

                    updateInvoiceKioskoInformation(tranid, body_peticion, razon_social_to_send, clienteId,trantype);
                }
            } else {

                updateInvoiceKioskoInformation(tranid, body_peticion, razon_social_to_send, body_peticion.cliente.id,trantype);
            }


            var SLURL = urlMod.resolveScript({
                // MOD: 13/01/2024 intento de nuevo script para timbrar facturas de kiosko
                scriptId: 'customscript_mx_plus_xml_generator_kiosk',
                // scriptId: 'customscript_efx_fe_xml_generator',
                deploymentId: 'customdeploy_mx_plus_xml_generator_kiosk',
                ...
                params: {
                    ...
                    trantype: tipodeTran,
                    tranid: tranid,
                    razon_social: razon_social_to_send,
                    rfc: body_peticion.cliente.rfc,
                    codigo_postal: body_peticion.direccion.cp,
        ...
        function removerAcentos(texto) {
            ...
            // Reemplazar acentos
            var textoSinAcentos = texto.replace(/[áéíóúÁÉÍÓÚ]/g, function (letra) {
                return mapaAcentos[letra] || letra;
            });

            // Eliminar dobles espacios
            var textoSinDoblesEspacios = textoSinAcentos.replace(/\s+/g, ' ');

            // Eliminar espacios al inicio y al final
            var textoSinEspaciosAlInicioYFin = textoSinDoblesEspacios.trim();

            return textoSinEspaciosAlInicioYFin;
        }
        ...
        function buscarCliente(obj_cliente, rfc, creaFactura, SUBSIDIARIES) {
            ...
            var search_ticket = search.create({
                ...
                filters: [
                    ...
                    , 'and',
                    ['custentity_mx_rfc', search.Operator.IS, rfc]
        ...
        function crearCliente(objcliente, document_pack, datos_clientes, versioncfdi) {
            ...
            if (objcliente.cliente.tipo) {
                // Aqui hay que poner en mayusculas el nombre
                var name_txt = objcliente.cliente.nombre + ' ' + objcliente.cliente.apellido
                // removerAcentos también quita dobles espacios
                razon_social_to_send = removerAcentos(name_txt.toUpperCase());
                record_client.setValue({
                    fieldId: 'entityid',
                    value: razon_social_to_send + ' ' + objcliente.cliente.rfc
                });

                record_client.setValue({
                    fieldId: 'firstname',
                    value: removerAcentos(objcliente.cliente.nombre.toUpperCase())
                });

                record_client.setValue({
                    fieldId: 'lastname',
                    value: removerAcentos(objcliente.cliente.apellido.toUpperCase())
                });
                ...
                // Aqui hay que poner en mayusculas el nombre en caso persona moral

                var company_txt = objcliente.cliente.compania
                var razon_social_to_send = removerAcentos(company_txt.toUpperCase());
                record_client.setValue({
                    fieldId: 'entityid',
                    value: razon_social_to_send
            ...
            var obj_old_client_values = {
                regimenFiscal_text: '',
                regimenFiscal_value: objcliente.cliente.regFiscal.value,
                nombre: objcliente.cliente.nombre,
                apellido: objcliente.cliente.apellido,
                compania: objcliente.cliente.compania,
                codigo_postal: objcliente.direccion.cp,
                calle: objcliente.direccion.calle,
                n_exterior: objcliente.direccion.n_exterior,
                n_interior: objcliente.direccion.n_interior,
                pais: objcliente.direccion.pais,
                estado: objcliente.direccion.estado,
                municipio: objcliente.direccion.municipio,
                colonia: objcliente.direccion.colonia,
        ...
        function crearFactura(transid, tipo, cliente_id, objeto, plantilla_f, clienteoriginal_fact, SUBSIDIARIES, campos_refacturacion, regFiscalSubsidiaria) {
            ...
            try {
                ...
                record_invoice.setValue({
                    fieldId: 'custobjeto_efx_fe_kiosko_address',
                    value: objeto.direccion.calle + ' ' + objeto.direccion.n_exterior + ' ' + objeto.direccion.n_interior + '\n' + objeto.direccion.colonia + '\n' + objeto.direccion.cp + ' ' + objeto.direccion.municipio + ', ' + objeto.direccion.estado + '\n' + objeto.direccion.pais,
                    ignoreFieldChange: true
                });
                log.audit({ title: 'id_cliente 🥰🥰🥰', details: id_cliente });
                log.audit({ title: 'objeto.cliente.regFiscal.value 🥰🥰🥰', details: objeto.cliente.regFiscal.value });
                //AKKK
                record_invoice.setValue({
                    fieldId: 'custbody_efx_fe_kiosko_regfiscal',
                    value: objeto.cliente.regFiscal.value,
                    ignoreFieldChange: true
                });
